SOURCES       = ../ffmulticonverter/ffmulticonverter.py ../ffmulticonverter/preferences_dlg.py
SOURCES      += ../ffmulticonverter/progress.py ../ffmulticonverter/tabs.py ../ffmulticonverter/presets_dlgs.py
SOURCES      += ../ffmulticonverter/about_dlg.py
TRANSLATIONS  = ffmulticonverter_en.ts ffmulticonverter_el.ts ffmulticonverter_tr.ts
TRANSLATIONS += ffmulticonverter_ru.ts ffmulticonverter_zh_CN.ts ffmulticonverter_de_DE.ts
TRANSLATIONS += ffmulticonverter_hu.ts ffmulticonverter_pt.ts ffmulticonverter_pl_PL.ts
TRANSLATIONS += ffmulticonverter_cs.ts ffmulticonverter_pt_BR.ts
